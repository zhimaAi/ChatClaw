...
func (t *scheduledTaskCreatePreviewTool) Info(_ context.Context) (*schema.ToolInfo, error) {
	return &schema.ToolInfo{
		...
		ParamsOneOf: schema.NewParamsOneOfByParams(map[string]*schema.ParameterInfo{
			...
			"schedule_type":  {Type: schema.String, Desc: selectDesc("Schedule type: 'preset' (only fixed values: every_hour, every_day_0900, weekdays_0900, every_monday_0900) or 'cron' (with cron_expr)", "调度类型: 'preset'(仅支持固定值: every_hour, every_day_0900, weekdays_0900, every_monday_0900) 或 'cron'(需提供cron_expr)"), Required: true},
