---
name: 长期记忆方案设计
overview: 基于 ChatClaw 现有 Eino + SQLite + sqlite-vec 基础设施，构建全局长期记忆系统。记忆自动从对话中提取（后台异步），用户可在独立的「记忆管理」页面（侧边栏知识库下方）查看、编辑、删除、手动添加。所有 Agent 共享同一个全局长期记忆池，通过 memory_search Tool 在对话中自动检索。
todos:
  - id: migration
    content: 创建 memories + mem_vec + mem_fts 数据库迁移文件
    status: pending
  - id: memory-model
...
# ChatClaw 长期记忆（Long-Term Memory）方案
...
### 第四步：Pre-Compaction Memory Flush
...
- `internal/sqlite/migrations/202602071000_create_memories_table.go` — 数据库迁移（memories + mem_vec + mem_fts）
