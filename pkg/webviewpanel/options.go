...
// This interface is designed to be compatible with the official wails PR #4880
// for seamless migration once merged.
type WebviewPanelOptions struct {
	...
	//
	// Note: this repo's custom PanelManager currently does not auto-apply anchors
	// on native window resize; we keep this field for API parity with wails PR #4880.
	Anchor AnchorType
...
// NewRGB creates a new RGBA color with full opacity.
func NewRGB(r, g, b uint8) RGBA {
	return RGBA{Red: r, Green: g, Blue: b, Alpha: 255}
}

// NewRGBA creates a new RGBA color with the specified alpha.
