export default {
  ...
  channels: {
    ...
    unbindSuccess: "已解绑助手",
    wechat: {
      emptyTitle: "WeChat アカウントはまだ追加されていません",
      emptyDesc: "Tencent 公式 OpenClaw プラグインを使って QR コードをスキャンし、個人用 WeChat を接続して WeChat メッセージの受信と処理を始めます。",
      addNow: "今すぐ追加",
      configTitle: "WeChat を設定",
      configSubtitle: "Tencent 公式 OpenClaw プラグインを使って QR コードをスキャンし、個人用 WeChat を接続します",
      howToConnect: "接続方法",
      step1: "Tencent 公式 OpenClaw プラグインを使って QR コードをスキャンし、個人用 WeChat を接続します。",
      step3: "「QR コードを生成」をクリックします。ClawX が OpenClaw に公式 WeChat プラグインをインストールして有効化します。",
      step4: "下の QR コードを WeChat でスキャンし、スマートフォンで接続を確認してください。",
      step5: "接続が完了すると、WeChat に「WeChat ClawBot」という新しいチャットが自動で表示されます。同じ QR フローを繰り返すことで、さらにアカウントを追加したり既存アカウントを再接続したりできます。",
      configStepsLink: "設定ガイド",
      ...
      assistantPromptDesc: "デフォルトではメインアシスタント（main）が関連付けられ、受信メッセージを処理します。別のアシスタントを使いたい場合は、既存のアシスタントを関連付けるか、新しく作成してこのチャネルに自動で関連付けることができます。",
      ...
      missingChannelId: "チャネル情報を取得できませんでした。このダイアログを閉じてチャネル一覧を更新し、もう一度お試しください。",
      missingChannelIdHint: "チャネル ID を取得できませんでした。このダイアログを閉じ、チャネル一覧から手動でアシスタントを関連付けてください。",
      channelNotFound: "対応するチャネルが見つかりませんでした。更新してもう一度お試しください。",
      pluginInstallTryLater:
        "公式 WeChat プラグインは現在バックグラウンドでインストールまたは有効化されています。しばらくしてからもう一度お試しください。",
      editNotSupported:
        "WeChat は QR コードで接続されるため、ここでは編集できません。アカウントを切り替えるには、チャネルページでこのチャネルを削除してから再度スキャンして追加してください。",
    },
  },
  common: {
    all: "全部",
