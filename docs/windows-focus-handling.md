...
### 吸附目标一致性排查（2026-02 经验）

症状（高频出现于多目标应用并行时）：

1. 控制状态显示 `currentTarget=A`，但 winsnap 视觉上贴在 `B`
2. 前台应用已切换到目标 `A`，winsnap 仍不回归
3. 快速切换时出现“来回闪动”或短时卡顿

根因分类：

1. **状态与运行时附着错位**：`currentTarget` 与控制器实际跟随窗口不一致
2. **切换并发竞争**：多次 `attachTo` 在短时间内交错，导致控制器被覆盖
3. **前台抖动链路**：目标切换瞬间，焦点/唤醒链触发多次附着切换

稳态策略（已验证）：

1. `attachTo` 必须串行化（单入口互斥），禁止并发切换控制器
2. 吸附态只做低频纠偏，不做高频全量回查（避免性能抖动）
3. 切换需有最短间隔（cooldown），抑制 A/B 抖动切换
4. 目标切换后短时间禁止 `WakeAttached`（打断唤醒连锁）
5. 低频检查必须校验“状态目标是否仍与实际附着一致”

排查优先级：

1. 先确认 `currentTarget` 与实际附着是否一致（而非只看前台识别）
2. 再确认是否存在切换并发（同一窗口周期内多次 `attachTo`）
3. 最后再调整前台识别和回查频率
...
| 2026-02-27 | 新增“吸附目标一致性排查”章节，记录状态/附着错位、切换并发与唤醒连锁的诊断和防回归策略 |
