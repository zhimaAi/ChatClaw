...
本文档梳理 ChatClaw 在 **macOS** 上与窗口相关的 4 个对象之间的层级（z-order / window level）与唤醒（activate / focus）策略：

- **主窗体**：`main`（ChatClaw 主界面）
- **吸附窗体**：`winsnap`（ChatClaw 侧边吸附聊天窗）
...
- **互动**：哪些用户动作会触发窗口之间的联动？
- **唤醒**：哪些场景会激活目标应用、激活 ChatClaw、以及把键盘焦点交给哪个窗口？
...
### 场景 D：用户在 ChatClaw 自己窗口内划词（main/winsnap）

问题背景：

- macOS 的系统级 mouse hook 会跳过“我们自己是 frontmost”的情况（避免递归/权限/事件干扰）
- 因此在 ChatClaw 内部划词需要前端兜底监听 selection
